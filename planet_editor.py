...
class World:
    ...
    def update(self):
        for human in self.humans:
            ...
            for target in human.targets:
                if not target.dead and (human.x, human.y) == (target.x, target.y):
                    self.fight(human, target)
...
class Human:
    def __init__(self, world, x, y, gender):
        ...
        self.history = []  # History of events
        ...
        if 0 <= new_x < self.world.grid_size and 0 <= new_y < self.world.grid_size:
            ...
            if cell.resource:
                if self.group:
                    ...
                    cell.resource = False  # Collect the resource
                    self.resources += 1
                self.history.append("Collected resource at ({}, {})".format(self.x, self.y))
    ...
    def join_group(self, group):
        ...
        self.history.append("Joined group")
                ...
                if event.button == 1:  # Left mouse button
                    ...
                    for human in self.world.humans:
                        if human.x == x and human.y == y:
                            self.show_human_info(human)

    def show_human_info(self, human):
        print("Human Info:")
        print("Gender:", human.gender)
        print("Health:", human.hp)
        print("Resources collected:", human.resources)
        print("History of Events:")
        for event in human.history:
            print("-", event)
        print("")
